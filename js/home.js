...
async function loadInitialCases() {
    ...
    try {
        const response = await fetch(`${API_URL}/api/case`, { // Rota para buscar todos os casos (sem filtro)
            method: "GET",
            headers: {
...
async function searchCasesByName() {
    ...
    const token = localStorage.getItem("token");
    try {
        const response = await fetch(`${API_URL}/api/case/fname?nameCase=${encodeURIComponent(searchTerm)}`, { // Use encodeURIComponent para termos com espaços/caracteres especiais
            headers: { Authorization: `Bearer ${token}` }
        });
        if (!response.ok) {
